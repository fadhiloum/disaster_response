...
function mockDataRepository(
  ...
  const data = {
    ...
    upsertIncidentConceptNote: vi.fn(async () => savedConceptNote),
  };
...
describe("concept note route", () => {
  ...
  it("persists a reviewed concept note draft", async () => {
    ...
    expect(data.upsertIncidentConceptNote).toHaveBeenCalledWith({
      incidentId: incident.id,
  ...
  it("returns 400 when concept note content is missing", async () => {
    ...
    expect(data.upsertIncidentConceptNote).not.toHaveBeenCalled();
  });
